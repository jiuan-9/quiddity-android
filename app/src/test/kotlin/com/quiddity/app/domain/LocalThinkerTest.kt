...
class LocalThinkerTest {
    ...
    fun think_toolIntent_mentionsQueryingFirst() {
        ...
        assertTrue(out.contains("查一下"), "涉及手机信息应提到先查证")
        assertFalse(out.contains("用户想："))
    }

    @Test
    fun think_greeting_usesAiName() {
        val out = LocalThinker.think("嗨", false, "小美")
        assertTrue(out.contains("小美"), "打招呼思考应带人设名")
        assertTrue(out.contains("回应"), "打招呼思考应自然")
    }
