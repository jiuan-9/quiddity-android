...
class ShizukuClient(private val context: Context) : ShizukuShell {
    ...
    /**
     * 申请 Shizuku 授权并等待结果：已授权立即返回 true；Shizuku 未运行返回 false；
     * 未授权时拉起系统授权弹窗，等待用户决定（60 秒超时按 false 处理）。
     */
    override suspend fun requestPermissionAndWait(): Boolean {
        if (isGranted()) return true
        if (!Shizuku.pingBinder()) return false
        return withTimeoutOrNull(PERMISSION_WAIT_TIMEOUT_MS) {
            suspendCancellableCoroutine { cont ->
                val started = requestPermission { granted ->
                    if (!cont.isCancelled) cont.resume(granted)
                }
                if (!started && !cont.isCancelled) cont.resume(false)
            }
        } ?: false
    }
    ...
    override suspend fun exec(command: Array<String>): AgentShellResult {
        if (!isGranted() && !waitForReady()) {
            return AgentShellResult(
                -1,
                if (Shizuku.pingBinder()) {
                    "未获得 Shizuku 授权"
                } else {
                    "Shizuku 未运行或连接已断开，请确认 Shizuku 已启动后重试"
                }
            )
        }
        return shellMutex.withLock {
            withContext(Dispatchers.IO) {
                runCatching {
                    ...
                    // 连接可能已失效：清空缓存，让下一次调用重新绑定
                    boundService.set(null)
                    AgentShellResult(-1, e.message ?: "执行失败")
                }
            }
        }
    }
    ...
    /**
     * 等待 Shizuku binder 就绪（进程刚启动时连接可能尚未建立，避免误报“未授权/未运行”）。
     */
    private suspend fun waitForReady(): Boolean {
        var waited = 0L
        while (waited < BINDER_RECOVER_WAIT_MS) {
            if (isGranted()) return true
            delay(BINDER_RECOVER_POLL_MS)
            waited += BINDER_RECOVER_POLL_MS
        }
        return isGranted()
    }
    ...
    private suspend fun bindAndGet(): IRemoteShell {
        ...
        repeat(BIND_RETRY_COUNT) { attempt ->
            ...
            try {
                ...
                pendingBind = null
                lastError = "发起绑定失败：${e.message ?: e.javaClass.simpleName}"
                boundService.get()?.let { return it }
                if (attempt < BIND_RETRY_COUNT - 1) {
            ...
            try {
                ...
                lastError = "连接超时（${BIND_TIMEOUT_MS / 1000} 秒无响应）"
            } catch (c: CancellationException) {
                throw c
            } catch (t: Throwable) {
                lastError = "绑定失败：${t.message ?: t.javaClass.simpleName}"
            } finally {
                pendingBind = null
            }
            boundService.get()?.let { return it }
        ...
        throw IllegalStateException(
            "连接 Shizuku 用户服务失败（${lastError ?: "未知原因"}），" +
                "已自动重试 $BIND_RETRY_COUNT 次；请确认 Shizuku 正在运行，" +
                "若仍失败请强制停止本应用后重新打开"
        )
    }
    ...
    companion object {
        ...
        private const val BINDER_RECOVER_WAIT_MS = 3_000L
        private const val BINDER_RECOVER_POLL_MS = 300L
        private const val PERMISSION_WAIT_TIMEOUT_MS = 60_000L
